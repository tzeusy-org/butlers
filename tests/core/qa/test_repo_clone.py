...
async def test_ensure_cloned_reclones_when_repo_url_changed(tmp_path):
    ...
    clone_calls = [call.args for call in mocked.call_args_list if call.args[1:2] == ("clone",)]
    assert len(clone_calls) == 1
    assert clone_calls[0][-2] == "https://github.com/tzeusy-org/butlers"


@pytest.mark.asyncio
async def test_ensure_cloned_replaces_clone_when_origin_probe_fails(tmp_path):
    """An unreadable existing remote cannot be trusted as the configured repository."""
    clone_dir = tmp_path / "qa-repo"
    (clone_dir / ".git").mkdir(parents=True)
    (clone_dir / "stale-marker").write_text("old clone")
    configured_url = "https://github.com/tzeusy-org/butlers"
    clone = ManagedRepoClone(pool=_pool(configured_url), clone_dir=clone_dir)
    run = _fake_subprocess(
        [
            (b"", b"origin unavailable", 2),
            (b"", b"", 0),
        ]
    )
    ...
    clone_calls = [call.args for call in mocked.call_args_list if call.args[1:2] == ("clone",)]
    assert len(clone_calls) == 1
    assert clone_calls[0][-2] == configured_url


@pytest.mark.asyncio
async def test_ensure_cloned_reports_clone_failure_after_replacing_stale_clone(tmp_path):
    """A failed replacement clears the stale clone instead of retaining its old remote."""
    clone_dir = tmp_path / "qa-repo"
    (clone_dir / ".git").mkdir(parents=True)
    (clone_dir / "stale-marker").write_text("old clone")
    clone = ManagedRepoClone(
        pool=_pool("https://github.com/tzeusy-org/butlers"), clone_dir=clone_dir
    )
    run = _fake_subprocess(
        [
            (b"https://github.com/Tzeusy/butlers\n", b"", 0),
            (b"", b"clone denied", 128),
        ]
    )

    with pytest.raises(RuntimeError, match="git clone failed"):
        with patch("butlers.core.qa.repo_clone.asyncio.create_subprocess_exec", side_effect=run):
            await clone.ensure_cloned()

    assert not clone_dir.exists()
    assert clone.clone_path is None
