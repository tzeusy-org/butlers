...
- `recall()`/`search()` in `src/butlers/modules/memory/search.py` now enforce the same server-held read ceiling (`CatalogReadPolicy`/`resolve_catalog_read_policy`, loaded via `load_catalog_read_policy(pool)` from `runtime_config.catalog_read_sensitivity`) as `search_catalog`'s cross-butler reads — generalized to local `facts`/`rules`/`episodes` tables, applied in SQL via `allowed_sensitivities` on `semantic_search`/`keyword_search`/`hybrid_search` (opt-in at that layer; mandatory, auto-loaded-if-omitted at `recall`/`search`). `memory_context` (`tools/context.py`) loads this policy exactly once per assembly and threads it through Profile Facts, Task-Relevant Facts (recall), and Recent Episodes so nothing in one assembled context block can disagree with another; Profile Facts reports an excluded-by-ceiling count as `withheld: N` rather than silently dropping rows. An explicit `filters={"sensitivity": ...}` above the caller's ceiling raises `SensitivityAuthorizationError`, never an empty list. `bu-2jtfw.3` PR1 covers only this ceiling and the health-write sensitivity classification above; calibrated credence/precedence (owner-asserted facts dominating llm-inferred ones) and the Profile Facts `knowledge_class` receipt-feed cleanup are deliberately deferred to a follow-up bead.
