...
- `core_groups` uses split authority after core_232: Git `[butler.runtime_seed].core_groups` declares capability, while `runtime_config` may retain only a strict subset carrying a non-blank `core_groups_narrowing_reason`; every unreasoned/broadening mismatch reconciles to Git and audits once per `(butler, TOML digest)`. Diagnose tool drift through the daemon status snapshot and Butler Management three-way diff (declared/effective/registered names), not the DB row alone. Relationship's complete module surface is 66 group-owned tools plus the unconditional approval-dispatch `relationship_assert_fact` handler (67 total); never remove or hide that handler to satisfy the stale 66-total count. Same evidence rule applies to fleet numbers: verify the window and unit before quoting a dashboard aggregate.
