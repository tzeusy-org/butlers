...
- Frozen-seed trap (2026-09 JARVIS run 13; four independent lenses rediscovered it): `runtime_config.seed_if_empty` captured every butler's `core_groups` once (2026-04-07, `ON CONFLICT DO NOTHING`) and `daemon.py` reads that row, not `butler.toml`, so a group added to a butler's TOML since then is silently never registered (Finance/Relationship expose zero cross-butler tools; Switchboard never registers `open_case`; Relationship runs 26 of 66 declared tools). Before concluding "tool X is missing" or "butler Y cannot call Z", diff `SELECT butler, core_groups FROM public.runtime_config` against the TOML — the live tool surface is the row. Same shape for fleet numbers: connector "lifetime ingested" counts on `/ingestion` are Prometheus `_created` epoch timestamps summed as counts (`heartbeat.py:343-370`), and `/api/sessions/aggregate` silently ignores `period=` (all-time numbers under a weekly label) — verify a number's window and unit before quoting it.
