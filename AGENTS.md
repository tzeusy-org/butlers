...
### Owner-entity approval carve-out: scoping + cross-schema recognition (RFC 0017 §2.7)
...
- The approval gate's owner detection reads `relationship.entity_facts`, which non-relationship butlers (messenger/home) cannot — schema isolation via `SET ROLE butler_<schema>_rw` (`src/butlers/db.py`). Owner-directed sends from those butlers therefore parked as "unresolvable target" unless they use the narrow fallback: `public.resolve_owner_triple(predicate, candidates[])` (migration `core_145`) performs the owner lookup as a SECURITY DEFINER, and `identity.resolve_owner_channel_via_definer()` calls it when normal resolution returns None. Generic gates and the email-specific inline guard both use that fallback. The channel→predicate mapping + telegram normalization stay in Python to avoid SQL/Python drift.
- Owner-channel authorization must test ambiguity across all live matching entities before filtering to the owner role. Otherwise an identifier attached to both the owner and an external entity looks owner-only after filtering and can bypass approval. The `core_233` replacement for `public.resolve_owner_triple` performs that test, and every channel gate delegates direct resolution plus owner corroboration to `identity.resolve_channel_contact_with_owner_corroboration()` rather than reimplementing the decision. Direct `entity_id` gate resolution must independently exclude merged/deleted entities before trusting the owner role. Every current and future communication channel uses the same rule: exactly one active literal identifier on one live, non-merged, non-deleted owner entity receives the bypass; primacy remains routing metadata and never controls outbound authorization. External, unknown, ambiguous, inactive, merged, deleted, malformed, and lookup-error targets fail closed.
...
- Gate resolution order: (1) extract `(channel_type, channel_value)` or `contact_id` from tool args → (2) resolve the normalized candidates to exactly one active identifier on one live entity → (3) auto-approve only if that entity has the `owner` role, independent of channel primacy; otherwise check standing rules or park.
...
- `src/butlers/daemon.py::_register_core_tools()` delegates registration to `butlers.core_tools.register_all_core_tools()` through the effective `core_groups` decorator. Keep group, type, name, and direct-registration gates in those owning registration modules; do not recreate a second catalog in the daemon or tests.
- `tests/contracts/test_tool_surface_isolation.py::test_core_tools_catalog_completeness` collects actual dispatcher registrations across domain, Switchboard, Messenger, and Chronicler contexts. It guards the 79 unique registrations (71 group-decorated across 14 groups plus 8 direct registrations) and the role/name gates; `tests/daemon/test_daemon.py::test_all_core_tools_registered` protects the daemon-start integration seam.
...
### Vision / attachment tool result contract (bu-2jtfw.7)
- No tool result may carry a `data_base64` field over 64KB (`src/butlers/tools/attachments.py::MAX_INLINE_BASE64_BYTES`) — a multi-MB base64 string dumped into a JSON tool result was the anti-pattern this fixed. `get_attachment(storage_ref)` embeds base64 only under that cap and returns a typed `status: "refused"` result above it; `attachment_view(storage_ref)` is the correct path for `image/*` attachments — it returns a real MCP image content block (`fastmcp.utilities.types.Image`, auto-converted by FastMCP's `Tool.run()`), never JSON. `tests/core/test_attachment_view.py::test_no_other_tool_source_embeds_a_data_base64_field` is an AST guard enforcing this repo-wide.
- `roster/switchboard/tools/routing/contracts.py::IngestPayloadV1.normalized_text` is deliberately not a `NonEmptyStr`: a captioned-media message's normalized_text is the caption, legitimately `""` for a captionless photo/document. A connector must never synthesize a placeholder like `"[Photo]"` into normalized_text — see `telegram_bot.py::_extract_normalized_text` for the pattern (only photo/document skip the media-type-descriptor tier; other media types like stickers/voice still get a synthesized `[Label]`, which is fine since nothing consumes them as vision input).
- Per-media-id idempotency (one blob per Telegram photo/document, replay-safe) uses a two-layer check mirroring `gmail.py`'s `fetch_attachment` pre-put pattern: a process-local dict cache first, then (if `db_pool` is set) `switchboard.media_refs` — a generic `(connector_type, endpoint_identity, external_message_id, media_id)`-keyed ledger (migration `sw_034`), deliberately not a reuse of Gmail's `attachment_refs` table (that PK is `(message_id, attachment_id)` with no connector namespace — a cross-provider id collision is a real risk since both are opaque provider-assigned strings).
- `ModelFeature.VISION` (`src/butlers/core/model_capabilities.py`) is deliberately undeclared at every adapter baseline — vision support is a per-model fact within one adapter, not an adapter-wide one — so it stays `Support.UNKNOWN` (fails closed) until a `public.model_catalog` row declares `capabilities: {"vision": true}`. `Spawner._run` derives the requirement itself: any `attachments` entry with a `media_type` starting `"image/"` adds `ModelFeature.VISION` via `derive_dispatch_intent(..., extra_required_features=...)`.
...
- **Frontend tests ARE runnable locally in a worktree** (corrected 2026-09-09, bu-poe4s; the prior "no agent can run a frontend test" claim was stale). `frontend/node_modules` in the repo root is an empty directory, but `scripts/setup_worktree.sh` detects that (bu-87osw: content-check, not just `-e`) and falls back to a local `npm install` inside the worktree's own `frontend/` instead of symlinking a dead cache — this is expected, not an error, and is allowed. Verified empirically end-to-end in a fresh worktree: the fallback install took ~8s for 1317 packages (~650MB, gitignored, one-time per worktree), and every step of CI's `frontend` job then ran for real and passed against it — `npm run lint` (~49s), `lint:emdash`/`lint:query-coercion` (~5s each), `knip` (~2s), `npm run build`/`tsc -b` (~38s, 0 errors), and `npm run test` (full vitest suite, 494 files / 7852 tests, ~2min, all green) — roughly 4 minutes wall-clock total. Vitest snapshot regeneration (`vitest run -u`) therefore also works now: a stale `.snap` from a DOM-shifting change no longer needs hand-editing or owner escalation, just a real local run. Not verified in this pass: `frontend-e2e` (Playwright) needs a separate `playwright install --with-deps chromium` browser fetch, so treat that job's local runnability as unconfirmed rather than assuming it follows the same path.
...
- Postgres `INSERT ... ON CONFLICT (cols) DO UPDATE` targeting a **partial** unique index must repeat that index's exact `WHERE` predicate in the `ON CONFLICT` clause (`ON CONFLICT (cols) WHERE <predicate> DO UPDATE ...`) or it fails with `InvalidColumnReferenceError: there is no unique or exclusion constraint matching the ON CONFLICT specification` even though the matching partial unique index exists — Postgres will not infer the predicate for you.
- asyncpg does not accept an ISO date/datetime **string** for a `date`/`timestamptz`-typed bound parameter, even with an explicit `::date`/`::timestamptz` cast in the SQL text — the client-side binary codec encodes the Python value based on the parameter's inferred OID before the server ever evaluates the cast, raising `AttributeError: 'str' object has no attribute 'toordinal'`. Bind a real `datetime.date`/`datetime.datetime` object (`date.fromisoformat(...)`), never a string, regardless of any `::` cast present.
- `public.expected_signals` (core_210/211, `src/butlers/core/expected_signals.py::upsert_expected_signal`) is a general-purpose "we expect this input and don't have it" honesty ledger, not just connector-liveness plumbing: any butler role granted access (e.g. `butler_travel_rw`) can register a static-reference-data gap as `producer="owner"`, `producer_endpoint_identity=None`, `last_observed_at=None` — evaluates to `absent` (expected but never supplied) without fabricating a false "measured safe" claim.
- `roster/travel/tests/*.py` (and most roster test suites) hand-write their own minimal schema DDL per file rather than running the real Alembic chain; a new table or column added to a butler's migration must also be added to every test file's local `CREATE TABLE`/`ALTER TABLE` fixture that exercises code touching it, or tests fail with `UndefinedTableError`/`UndefinedColumnError` despite the migration being correct. For a table owned by another chain (e.g. `pending_actions` from `approvals`), prefer the shared stand-in in `src/butlers/testing/schema_standins.py` (`PENDING_ACTIONS.ddl()`) over hand-rolling it — a drift guard (`tests/config/test_schema_standin_parity.py`) refuses a new hand-written copy of an already-registered stand-in table.
- **A `vi.mocked(hook).mockImplementation(() => ({...}))` that builds a fresh object literal on every call is a deterministic frontend-CI-hang generator, not a flaky one.** Root-caused for bu-fjryy9 (the bu-qaisp jump-to-message hang): `ChatPanel`'s `useConversationTurn` (and its pre-refactor inline equivalent) has a `useEffect(() => {...}, [activeConversationId, messagesData, streaming])` that compares `messagesData` by reference, exactly the guarantee TanStack Query gives in production. A per-call-fresh mock breaks that guarantee, so the effect re-fires every render, its `setState` triggers another render, `messagesData` is STILL a new reference, and React Testing Library's `act()` spins its synchronous effect-flush loop forever — 100%+ CPU, zero test output, and vitest's own per-test timeout never fires because the loop never yields to the event loop (confirmed via V8 inspector: repeated `Debugger.pause` mid-hang landed inside `workLoopSync`/`renderWithHooks` every time, never blocked I/O). This is a test-authoring bug, not a production or CI/vitest infra bug, and it can silently hang every test *after* it in the same file even though those earlier tests never touch the offending mock (the hang lands on whatever test happens to run right after the describe block that installs the bad mock, which reads as if an unrelated earlier test is the culprit). Fix: pre-build one stable result object per branch outside the `mockImplementation` callback and return that same reference, exactly like the established `mockHooksForConversationRefetchGap()` pattern in `ChatPanel.test.tsx` — never construct the returned object inline inside the callback. To debug a suspected hang like this locally: run the file directly with `npx vitest run <file> --reporter=verbose`, note the last completed test line, then if it CPU-pegs, find the worker PID (`ps aux | grep 'node (vitest'`), attach with a small Node script opening a WebSocket to `http://127.0.0.1:9229/json`'s `webSocketDebuggerUrl` (SIGUSR1 arms the inspector on an already-running process), and repeatedly send `Debugger.pause` — a busy-loop's stack samples the SAME few frames (React/render internals) every time, distinguishing it from a single stuck native call.

- RFC 0002 Amendment 1 and RFC 0027 distinguish the complete registered MCP catalog from the initially loaded model working set. The 30-50 target constrains initial presentation, not total registered handlers; reconcile older roster spec/test ceilings against that accepted contract before deleting tools or raising limits.

- `core_groups` uses split authority after core_232: Git `[butler.runtime_seed].core_groups` declares capability, while `runtime_config` may retain only a strict subset carrying a non-blank `core_groups_narrowing_reason`; every unreasoned/broadening mismatch reconciles to Git and audits once per `(butler, TOML digest)`. Diagnose tool drift through the daemon status snapshot and Butler Management three-way diff (declared/effective/registered names), not the DB row alone. Relationship currently enables eight groups with 58 group-owned tools plus the unconditional approval-dispatch `relationship_assert_fact` handler (59 total). Its mixed `entity` group remains pruned until the adopted six-read/two-write split is implemented; never activate the two writes as a side effect of exposing the reads. Same evidence rule applies to fleet numbers: verify the window and unit before quoting a dashboard aggregate.
- `store_fact`'s `sensitivity` parameter defaults to `'normal'` when a caller simply omits it — NULL is never actually stored, so "NULL reads as normal" is a read-time fallback, not the real write-time failure mode. The real failure mode is a domain write path (e.g. `roster/health/tools/conditions.py::condition_add`/`medications.py::medication_add`) never passing `sensitivity=` at all, silently inheriting the least-restrictive default for data that should be classified higher. There is no fleet-wide default map from predicate to sensitivity; each domain write path must pass its own classification explicitly (`roster/health/tools/_helpers.py::HEALTH_SENSITIVITY_CONFIDENTIAL` is the health-butler instance of this pattern). Audit new/changed `store_fact`/`store_rule` call sites for an implicit `'normal'` whenever the data being written is more sensitive than casual.
- `memory_search()`, `memory_recall()`, and `memory_get()` MCP closures must pass one server-held `CatalogReadPolicy` from the owning daemon's `runtime_config.catalog_read_sensitivity` into local facts/rules/episodes reads; a private memory schema (for example Chronicler's) must never substitute its own missing/stale `runtime_config` row. Low-level helpers remain opt-in for internal callers, but public retrieval paths are mandatory and fail closed. `memory_get()` applies allowed sensitivities in its atomic `UPDATE ... RETURNING`, so an above-ceiling UUID is absent and cannot bump reference metadata. `memory_context` loads one policy per assembly, reserves preamble and `withheld: N` text before fitting Profile Facts, and omits an unaffordable profile section rather than overflow or expose excluded content. An explicit higher `filters={"sensitivity": ...}` request raises `SensitivityAuthorizationError`, never an ambiguous empty result.
...
- A full dev Compose restart advances every daemon boot epoch and invalidates the prior receiver observation. The Dashboard observer must make a prompt first pass with bounded startup retries before resuming TTL/2 cadence; a full first sleep leaves the fleet falsely all-stale for minutes. Board `stale` is observation state, not operator policy: `set_butler_registry_policy(..., 'active')` cannot refresh it, so only projected policy-held eligibility offers Restore. The board projects `paused`, `quarantined`, and `review_required` policies as `eligibility='quarantined'`; do not infer the exact policy subtype from eligibility alone. Receiver probes do not emit the board's session event, so the board must reconcile every 30 seconds even while that socket is healthy, covering both loss and recovery of receiver health.
- Legacy Switchboard TTL transitions have two recorded reason names: read-time reconciliation writes `ttl_expired`, while the sweep writes `liveness_ttl_expired` (or `liveness_ttl_2x_expired` for quarantine). A provenance migration must match the transition ID, state, and exact timestamp as well as the writer-specific reason; matching only one reason can turn automatic staleness into a sticky `review_required` policy. Receiver-derived health and legacy `last_seen_at` remain separate until every routing, candidate, scheduler, and display reader cuts over.
- The receiver-derived route flag governs more than `route()`: candidate filtering, scheduled dispatch, trusted recovery notification admission, legacy TTL sweeps, and Dashboard board/heartbeat projections must agree. Compose base services also serve production and default the flag off; dev hotreload defaults it on for both daemon and Dashboard. Never retire the old heartbeat reporter while any deployed rollback path still needs legacy `last_seen_at` freshness.
...
- The dev daemon's Codex credential volume is separate from the host session. CLI 0.144.1 rejected `gpt-6-luna` and `gpt-6-sol` as unsupported, while a temporary 0.156.1 binary using the daemon's existing volume answered both; diagnose the pinned image CLI before attributing that error to account entitlement. Test the exact configured reasoning effort, and keep device-auth document projection strict across CLI upgrades.
- OpenCode Go's official model endpoint and `opencode models opencode-go` establish IDs, not workspace access. Dashboard and daemon CLI caches can differ; `opencode models opencode-go --refresh` refreshes the local cache. Probe proposed IDs through the daemon before enabling them: `deepseek-v4.1-flash` was listed but rejected by the workspace's non-Global privacy region. Add exact `pricing.toml` coverage before routing new IDs because unpriced candidates are treated as cost-neutral in the routing score.

### Image dispatch needs a per-row vision declaration (bu-syuk3z)
`capability_unknown:vision` in a `no_fitting_candidate` refusal means no enabled catalog row for that tier declares `capabilities.vision = true`; it is not a model outage. Vision is never inferred (not from adapter, vendor, or models.dev) — only an exact-path nonce probe justifies it (`core_248` records the probed rows; method in `docs/runtime/model-routing.md`). Images reach models **only** as the `attachment_view` MCP image block; no runtime adapter touches image bytes, so "does adapter X support vision" is a CLI + model question. OpenCode/Codex silently strip images for models their catalogs mark text-only (the model then answers "cannot see"), and Codex/Claude Code drop `content[]` entirely if a tool result carries `structuredContent` — never annotate `attachment_view`'s return type. Probing Codex directly (not via `CodexAdapter`, which requires the system-global auth authority) is safe only with a copied `auth.json` in a throwaway `CODEX_HOME` and a token far from refresh; a refresh would rotate the shared refresh token.
