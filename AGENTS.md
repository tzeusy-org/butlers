...
- Owner-auth database pools require a dedicated login whose full SET ROLE ancestry lacks host-function execution and privileged role flags. `SET LOCAL ROLE` and `NOINHERIT` alone do not remove the login's underlying authority; preflight restricted function OIDs through `pg_proc` before activating the capability role.
