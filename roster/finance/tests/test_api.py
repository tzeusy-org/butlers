...
@pytest.mark.asyncio
async def test_list_cost_claims_keeps_unreviewed_claim_without_fabricating_state():
    row = {
        "id": uuid.uuid4(),
        "claim_key": "loan:fact-unreviewed",
        "asserted_by": "relationship",
        "kind": "receivable",
        "direction": "inbound",
        "amount": Decimal("25.00"),
    ...
    app, pool = _make_app(fetch_rows=[row], fetchval_return=1)
    async with httpx.AsyncClient(
        transport=httpx.ASGITransport(app=app), base_url="http://test"
    ) as client:
        response = await client.get("/api/finance/cost-claims")
    ...
    assert response.status_code == 200
    assert response.json()["data"][0]["state"] is None
    assert "LEFT JOIN public.cost_claim_resolutions" in pool.fetch.call_args[0][0]
