...
def upgrade() -> None:
    ...
    op.execute(
        """
        CREATE INDEX idx_entity_rebind_log_target_status
        ON public.entity_rebind_log (target_schema, status, created_at)
        """
    )
    op.execute(
        """
        CREATE INDEX idx_entity_rebind_log_entities
        ...
        op.execute(
            ...
            BEGIN
                IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = '{role}') THEN
                    GRANT SELECT, INSERT, UPDATE ON public.entity_rebind_log TO "{role}";
                END IF;
            EXCEPTION
...
def downgrade() -> None:
    op.execute(
        ...
        DO $$
        BEGIN
            IF to_regclass('public.entity_rebind_log') IS NOT NULL
               AND EXISTS (SELECT 1 FROM public.entity_rebind_log)
            THEN
                RAISE EXCEPTION
