"""Index filtered event status and time without blocking ingestion writes.

Revision ID: core_245
Revises: core_244
Create Date: 2026-09-22 00:00:00.000000

The default Timeline histogram excludes ordinary filtered events. The existing
received_at index still visits every filtered row in the time window, while the
connector-leading index cannot narrow status without a connector predicate.
A status-leading time index bounds these reads to the selected statuses.
Build each leaf index concurrently, then attach it to an initially empty
partitioned index. PostgreSQL marks the parent valid after every partition is
attached and indexes future partitions itself.
...
revision = "core_245"
down_revision = "core_244"
branch_labels = None
depends_on = None

_INDEX = "connectors.ix_filtered_events_status_received"
_LOCK = 245_20260922
...
def upgrade() -> None:
    with op.get_context().autocommit_block():
        ...
        try:
            ...
            try:
                op.execute(
                    "CREATE INDEX IF NOT EXISTS ix_filtered_events_status_received "
                    "ON ONLY connectors.filtered_events (status, received_at DESC)"
                )
                ...
                for oid, schema, table in partitions:
                    ...
                    if attached:
                        continue
                    name = f"ix_filtered_events_status_received_{oid}"
                    ...
                    op.execute(
                        ...
                        f"ON {_quote(schema)}.{_quote(table)} (status, received_at DESC)"
                    )
