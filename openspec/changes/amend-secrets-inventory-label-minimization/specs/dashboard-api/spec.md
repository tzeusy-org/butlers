...
- **AND** the per-credential detail endpoints defined below are unaffected by this scenario: `GET /api/secrets/system/<key>` continues to publish the system fields `key`, `category`, and `description`, while `GET /api/secrets/cli/<id>` continues to publish the CLI identity fields `id` and `label` (where `label` is projected from the stored description); the CLI detail payload does not publish fields named `key`, `category`, or `description`
- **AND** the resulting inventory/detail asymmetry is deliberate rationale for this inventory-only proposal, not an authentication or authorization boundary: a caller commonly reaches a detail read after selecting an identified row, but neither detail endpoint gains a stronger caller-authentication guarantee from that navigation pattern; widening or narrowing either detail payload requires a separate proposal
...
- **AND** the current baseline clause "`GET /api/secrets/system/<key>` returns `ApiResponse<SystemSecret>` with `key`, `category`, `row_state` (one of `shared` / `local` / `missing`), `fingerprint`, `description`, `source`, `target`, `last_verified`, `used_by[]`, `breaks[]`, `test`, `audit[]`" is retained here only as superseded archive provenance; it SHALL NOT define the response while the implemented `SystemCredentialDetail` clause above and its sibling change are pending baseline adoption
- **AND** the current baseline clause "`GET /api/secrets/cli/<id>` returns `ApiResponse<CliRuntime>` with `id`, `label`, `fingerprint`, `state`, `issued`, `expires`, `last_used`, `scopes_required`, `scopes_granted`, `test`" is retained here only as superseded archive provenance; it SHALL NOT define the response while the implemented `CliCredentialDetail` clause above and its sibling change are pending baseline adoption
...
- **AND** the system detail payload continues to publish `key`, `category`, and `description`; the CLI detail payload instead continues to publish `id` and `label`, with `_fetch_single_cli_secret` projecting the stored `description` column as `label`, and SHALL NOT publish fields named `key`, `category`, or `description`
