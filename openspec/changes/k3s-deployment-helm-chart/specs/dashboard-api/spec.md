...
The dashboard-api SHALL expose a public, content-blind `GET /ready` endpoint (distinct from `/health`) that verifies PostgreSQL connectivity, expected Git-roster discovery, a fresh complete receiver-derived observer snapshot, verified current-generation identity and route acceptance for every expected non-paused daemon, compatible route contracts, QA patrol freshness, supervised control-plane loop state, and a content-free effect-free route canary. It SHALL return HTTP 200 with `{"ready": true}` only when all checks pass, or HTTP 503 with `{"ready": false, "checks": {...}}` when any check fails. Failure checks SHALL be fixed-name booleans for `postgres`, `roster`, `observer`, `fleet`, `qa_patrol`, `supervisors`, and `route_canary`; they SHALL NOT disclose daemon names, endpoints, credentials, event content, or raw error text. A failed or incomplete observation SHALL NOT become an all-clear. This requirement composes with `restore-butler-control-plane-liveness`; process startup or a discoverable roster row alone SHALL NOT establish fleet readiness.
