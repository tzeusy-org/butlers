...
After exact adoption and integration of the source-union contract at PR #4058
`652257065e7fda464979f6ce2bd76075052e4d78`, the Relationship activity route gains optional
...
Relationship selects both independently read local stores (narrative and identity).
...
shape compatible with that source-union prerequisite at the field level, including
`store` and `summary`; compatibility is not against the older identity-only source.
...
`ts DESC NULLS LAST, (src, COALESCE(store, ''), id) ASC` before the slice so a stable data set produces
stable offset pages. Every consumer keys rows by `(src, store, id)`, never UUID
alone; the episode door remains the Chronicler `episode_id`. `total` counts
only the selected readable candidate set returned by bounded source reads before
that slice, not unseen upstream history. Concurrent writes or changed upstream
bounds do not provide snapshot isolation across offset requests. When daily bins are requested, they are also computed from the complete
...
omits stream items and totals, retaining the same bins and degradation fields. The summary intentionally renders only the first five
...
wiring, change the source-union prerequisite
default `/activity` response, or remove Chronicler rows from
