...
- [ ] 0.4 Adopt PR #4058's exact source-union artifact before adopting this local
      derivative, then integrate source-union before integrating the derivative; re-read the then-current
      whole activity requirement and re-review any changed composition.
...
- [ ] 1.2 Extend the Relationship activity API tests to prove
      ...
      total, bins, timestamp plus source-qualified tuple ordering, and pagination;
      preserve equal-timestamp/cross-store UUID collisions without deduplication;
      both Relationship stores remain selected together; bounded source reads
      limit total claims; and `bins_only` uses the selected set while omitting
      items and totals. Cover valid empty, total failure, and
...
- [ ] 2.1 Add the optional activity `source` filter, apply it before pagination,
      and preserve the adopted source-union response when it is omitted, including
      `store`/`summary`, tuple identity, and the shared deterministic order.
