...
## Local reconciliation provenance

This local five-file derivative starts from PR #4052 at
`d1fb2007f529136f38a4efdcc4e2b49b0ae47c4e`; it is not that PR head and has not
been published. It composes the shared activity contract with proposed PR
#4058 at `652257065e7fda464979f6ce2bd76075052e4d78`. Adopt that source-union artifact before adopting this derivative; integrate
it before integrating this derivative. If source-union adoption is
held or its bytes change, hold this dependent adoption and refresh the composition.
No adoption is inferred from this dependency.

The reconciliation uses the source-union's deterministic source-qualified
ordering, includes `store` in the Chronicler projection, defines Relationship
selection as both independent local stores, limits totals to the selected
bounded candidate set, and preserves bins-only omission of items and totals.
It rebuilds the shared aggregator's whole requirement so the new selector
qualifies source execution and degradation rather than contradicting an
unconditional all-source clause. Attendee navigation, Chronicle episode doors,
commitment behavior, and all original non-goals remain unchanged.

## Compatibility and Authority

All wire changes are additive relative to the separately adopted source-union
prerequisite. Omitting the new activity `source` query keeps that merged response. Existing `/chronicles?date=...` links keep their
