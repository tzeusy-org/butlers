...
- **THEN** fixtures cover wrong or missing parent relkind/strategy/key, missing
  and extra attached children, unattached name matches, child-kind mismatch,
  lower/upper-bound mismatch, row-count mismatch, optional status mismatch,
  unknown status, and query/parse failure
