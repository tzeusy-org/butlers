...
- Define a fixed, content-blind scoped verdict that binds a pass to the artifact
  digest, manifest digest, and capture completion time. Any scoped failure makes
  the overall restore drill fail at its existing `verify` stage.
...
verification stage, protected result ledger interface, backup/restore operations
documentation, and focused script/job/integration tests. It must preserve the
existing isolated executor's file-secret, network, scratch-lifecycle, and
sanitized-result boundaries. This planning-only change changes no runtime
behavior.
